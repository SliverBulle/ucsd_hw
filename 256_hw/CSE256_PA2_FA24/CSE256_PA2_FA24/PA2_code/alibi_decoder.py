...
from adapted_transformer import TransformerEncoder, TransformerDecoder, FeedforwardClassifier
...
def compute_perplexity(decoderLMmodel, data_loader, eval_iters=100):
    ...
    with torch.no_grad():
        for X, Y in data_loader:
            ...
            #   logits and Y to match CrossEntropyLoss input format
            B, T, C = logits.shape
...
def main():
    ...
    plt.grid(True)
    plt.savefig('plot/alibi_decoder_train_perplexity.png')
    plt.close()
    # save decoder model
    decoder_file = 'model/alibi_decoder.pth'
    ...
    # compute perplexity of training set and test set
    train_perplexity = compute_perplexity(decoder, train_LM_loader, eval_iters=100)
    print(f"Training set perplexity: {train_perplexity:.2f}")
    ...
    plt.legend()
    plt.grid()
    plt.savefig('plot/alibi_decoder_test_perplexity.png')
